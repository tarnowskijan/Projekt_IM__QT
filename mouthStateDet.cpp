...
bool myAbsDiff2(Mat patternMat, Mat currMat, Mat *dest){
    if(patternMat.channels() > 1 || currMat.channels() > 1){return false;}
...
MouthState detectMouthState(Mat mouth, Mat patternUp, Mat patternDown,
    ...
    typedef bool (*ptrAbsDiff)(Mat, Mat, Mat*);
    //ptrAbsDiff funAbsDiff = myAbsDiff; //ta funkcja absDiff uzywa przycinania
    ptrAbsDiff funAbsDiff = myAbsDiff2;  //ta funkcja absDiff uzywa zmiany rozmiaru


    funAbsDiff(patternDown,mouth(r),&tmp);
    sadDown = sum(tmp)[0] / (255 * mouth(r).rows * mouth(r).cols);

    funAbsDiff(patternUp,mouth(r),&tmp);
    sadUp = sum(tmp)[0] / (255 * mouth(r).rows * mouth(r).cols);

    funAbsDiff(patternLeft,mouth(r),&tmp);
    sadLeft = sum(tmp)[0] / (255 * mouth(r).rows * mouth(r).cols);

    funAbsDiff(patternRight,mouth(r),&tmp);
    sadRight = sum(tmp)[0] / (255 * mouth(r).rows * mouth(r).cols);

    funAbsDiff(patternClick,mouth(r),&tmp);
    sadClick = sum(tmp)[0] / (255 * mouth(r).rows * mouth(r).cols);

    funAbsDiff(patternNeutral,mouth(r),&tmp);
